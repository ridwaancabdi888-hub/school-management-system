// Creates the schema and loads demo data for two isolated schools, against
// Supabase/Postgres. Run with: npm run db:setup
// (Parallel to the original database/setup.js, which does the same thing
// against MySQL and is kept for local-MySQL-dev reference — see README.)
...
async function run() {
  ...
  const admin = new Pool({ connectionString: env.db.url, ssl: env.db.ssl ? { rejectUnauthorized: false } : false, max: 1 });

  console.log('Applying schema...');
  const schemaSql = fs.readFileSync(path.join(__dirname, 'schema.sql'), 'utf8');
  await admin.query(schemaSql);
  await admin.query(fs.readFileSync(path.join(__dirname, 'security.sql'), 'utf8'));

  // Use the app's own adapter from here on so this script exercises the
  // exact same query/transaction shape the running app relies on.
  await admin.end();
  ...
  for (const def of schoolDefs) {
    ...
    const [schoolResult] = await db.query(
      `INSERT INTO schools (name, code, city, phone, email, status, package, website_enabled, brand_color, currency, academic_year, start_date)
       VALUES (?, ?, ?, ?, ?, 'active', 'standard', ?, ?, ?, '2025-2026', CURRENT_DATE)`,
      [def.name, def.code, def.city, def.phone, def.email, def.websiteEnabled, def.brandColor, def.currency]
    );
    ...
    for (const t of teacherUsers) {
      ...
      const [te] = await db.query(`INSERT INTO teachers (school_id, user_id, joining_date, status) VALUES (?, ?, CURRENT_DATE, 'active')`, [schoolId, u.insertId]);
    ...
    // Accountant + staff
    {
      ...
      await db.query(`INSERT INTO staff (school_id, user_id, designation, joining_date, status) VALUES (?, ?, 'accountant', CURRENT_DATE, 'active')`, [schoolId, u.insertId]);
    ...
    }
    {
      ...
      await db.query(`INSERT INTO staff (school_id, user_id, designation, joining_date, status) VALUES (?, ?, 'reception', CURRENT_DATE, 'active')`, [schoolId, u.insertId]);
    }
    ...
    for (const s of studentDefs) {
      const admissionNo = `${def.code}-${new Date().getFullYear()}-${String(seq).padStart(4, '0')}`;
      seq++;
      ...
      const [studentRow] = await db.query(
        `INSERT INTO students (school_id, user_id, admission_no, first_name, last_name, class_id, section_id, gender, dob, admission_date, phone, guardian_name, guardian_phone, address, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, '2017-04-12', CURRENT_DATE, ?, ?, '+1-555-0999', '123 Main St', 'active')`,
      ...
      const [sf] = await db.query(
        `INSERT INTO student_fees (school_id, student_id, fee_type_id, academic_year, amount_required, due_date)
         VALUES (?, ?, ?, '2025-2026', 500.00, (CURRENT_DATE + 30))`,
      ...
      await db.query(
        `INSERT INTO payments (school_id, student_id, student_fee_id, amount, payment_date, method, receipt_no, recorded_by)
         VALUES (?, ?, ?, 300.00, CURRENT_DATE, 'cash', ?, ?)`,
    ...
    // Finance
    await db.query(`INSERT INTO finance_records (school_id, type, category, amount, record_date, recorded_by) VALUES (?, 'expense', 'Utilities', 250.00, CURRENT_DATE, ?)`, [schoolId, adminResult.insertId]);
    await db.query(`INSERT INTO finance_records (school_id, type, category, amount, record_date, recorded_by) VALUES (?, 'income', 'Donation', 1000.00, CURRENT_DATE, ?)`, [schoolId, adminResult.insertId]);

    // Website content (only meaningfully used when website_enabled = true)
    await db.query(
